...
void Interact::Draw()
{
	...
	const float kSlotSize = 64.0f;
	const float kSlotY = SCREEN_HEIGHT - kSlotSize - 20.0f;
	const float kWarningY = kSlotY - 30.0f; // clearly above the slot row, not overlapping it
	...
	if (hasWarning)
	{
		Hud::DrawText(s_WarningText, SCREEN_WIDTH * 0.5f, kWarningY, 24.0f, true);
	}
