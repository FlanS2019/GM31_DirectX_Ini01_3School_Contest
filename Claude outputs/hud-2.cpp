...
void Hud::DrawFilledRect(float x, float y, float width, float height, float r, float g, float b, float a)
{
	if (!g_Ready) return;

	// STEP34: 呼び出しごとに単色ブラシを作って使い捨てる(ポーズ/設定
	// 画面が開いているときしか呼ばれない低頻度の描画なので、毎回作成
	// しても実用上問題ない)。
	ID2D1SolidColorBrush* brush = nullptr;
	HRESULT hr = g_D2DRenderTarget->CreateSolidColorBrush(D2D1::ColorF(r, g, b, a), &brush);
	if (FAILED(hr) || !brush) return;

	g_D2DRenderTarget->FillRectangle(D2D1::RectF(x, y, x + width, y + height), brush);
	brush->Release();
}