...
void Key::Init()
{
	// STEP25: リザルト画面の「見つけた鍵: N / M」用に、マップに配置された
	// 鍵の総数を数えておく(Game::Init()冒頭でGameStats::Reset()で毎回
	// ゼロに戻してから、Map::Init()経由でKeyが生成されるたびにここで+1する)。
	...
	modelRenderer->Load("model\\Key_Pickup.obj"); // STEP37: was model\key.obj (a plain box) -- see key.h
...
void Key::Update()
{
	...
	if (player)
	{
		...
		if (direction.length() < kPickupRange)
		{
			...
			player->PlayPickupSE(); // STEP21: "ものを取る音" -- see Player.h's m_PickupSE comment for why this plays from Player, not from Key (about to SetDestroy(true) below)

			// STEP25: door.cppのInteract()がロック解除時にPlayer::RemoveKey()で
			// このキーのインベントリビットを消してしまっても、「拾うまでに
			// 何本見たか」は減らしたくないので、Playerとは別にここに記録
			// しておく。
